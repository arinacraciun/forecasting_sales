...
# Plotting configuration: Enforce global seaborn styling
...
def run_imputation_experiment(train_final, output_dir='./graphs'):
    ...
    # 6. Plotting the winner using seaborn
    plt.figure(figsize=(12, 5))
    
    sns.lineplot(
        x=ts.loc[missing_window].index, 
        y=ts.loc[missing_window, 'sales_clean'], 
        label='True Sales', 
        linewidth=2, 
        color='silver'
    )
    ...
    sns.lineplot(
        x=ts.loc[missing_window].index, 
        y=ts.loc[missing_window, 'impute_seasonal'], 
        label='Seasonal Interpolation', 
        linestyle='--', 
        color='blue'
    )
...
def plot_diagnostics(train_df, output_dir='./graphs'):
    ...
    if len(available_features) > 0:
        ...
        # Plot and compare results using seaborn
        plt.figure(figsize=(15, 5))
        
        sns.lineplot(
            x=global_sales.index, 
            y=global_sales['sales'], 
            label='Normal Sales', 
            color='blue', 
            alpha=0.5
        )
        ...
        outliers_iqr = global_sales[global_sales['outlier_residual_iqr']]
        sns.scatterplot(
            x=outliers_iqr.index, 
            y=outliers_iqr['sales'], 
            color='red', 
            label='Residual IQR Outlier', 
            zorder=5,
            s=50
        )
        ...
        outliers_iso = global_sales[global_sales['outlier_iso_forest']]
        sns.scatterplot(
            x=outliers_iso.index, 
            y=outliers_iso['sales'], 
            color='orange', 
            marker='X', 
...
# Log-Transformation Comparison Plot
def plot_target_transformation(train_df, output_dir='./graphs'):
    """Plots the aggregate original sales vs log1p stabilized sales using seaborn."""
    print("Generating target transformation plot...")
    os.makedirs(output_dir, exist_ok=True)
